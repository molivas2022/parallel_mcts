    ...
    for (int game = 1; game <= matches; ++game) {
        ...
        while (state.winner == Player::None) {
            ...
            print_dashboard(past_results, config_num, total_configs, N, matches, 
                            ...
                            game, game_turns + 1, test_wins, baseline_wins, elapsed);
...
int main() {
    ...
    auto save_to_csv = [](const ExperimentResult& r, bool is_first) {
        ...
        if (file.is_open()) {
            if (is_first) {
                file << "N,Matches,Agent,Iters,Threads,Winrate,AvgTurns,Time_Seconds\n";
            }
    ...
    int total_configs = experiments.size();
    int current_config = 1;

    // Experiment loop
    for (const auto& config : experiments) {
        auto res = run_experiment(config, matches, baseline_agent, all_results, current_config++, total_configs);
